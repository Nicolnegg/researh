        ...
        for modell in model.split('\n'):
            if ':' in modell:
                ...
                if ';' in ldata[1] and not '(;)' in ldata[1]:
                    ...
                    result[ldata[0]] = ldata[1].split()[0].strip()
...
class BinsecCheckers(AbstractChecker):
    ...
    Temporary_Binsec_Configfile = 'temp.binsec.{}.config'
    ...
    def _check_dgoal_reachable_util(self, candidate, reject, complete=False):
        ...
        for example in reject:
            ...
            directives.append(self._generate_rejection_directive(example, op=directive_op))
    ...
    def _generate_rejection_directive(self, model, op=minibinsec.Operator.Or):
        return '{} assume {}'.format(self.addr, self.as_literal(model, coreop=minibinsec.Operator.Distinct, op=op))

    def _run_binsec_command(self, candidate, directives, formatted=False):
        self.stats.get_oracle('binsec').calls += 1
        if formatted:
            directives.append('{} assume {}'.format(self.addr, candidate))
        else:
            for assump in candidate:
                directives.append('{} assume {}'.format(self.addr, assump))
        self.config.set('sse', 'directives', ';'.join(directives))
        local_config_file = self._get_local_cfname()
        with open(local_config_file, 'w') as stream:
            self.config.write(stream)
        command = ['binsec', '-file', self.binary, '-config', local_config_file, '-sse-memory', self.args.binsec_memory]
        btime = time.time()
...
class RobustBinsecCheckers(BinsecCheckers):
    ...
    def _run_binsec_robust_command(self, candidate, directives, controlled, formatted=False):
        ...
        if formatted:
            ...
            for assump in candidate:
                directives.append('{} assume {}'.format(self.addr, assump))
        self.robust_config.set('sse', 'directives', ';'.join(directives))
        local_config_file = self._get_local_cfname()
        with open(local_config_file, 'w') as stream:
            self.robust_config.write(stream)
        ...
        command = ['binsec', '-file', self.binary, '-config', local_config_file, '-sse-memory', local_memory_file]
        btime = time.time()
        rc, to, out, err = execute_command(command, self.log, timeout=self.args.binsec_timeout)
        atime = time.time()
        if to:
    ...
    def _check_ngoal_unreachable(self, candidate):
        directives = [ d for d in self.directives['all'] ]
        directives.extend(self.directives['negative'])
        parser = self._run_binsec_robust_command(candidate, directives, self.var_engine.get_uncontrolled())
        ...
        if model is not None:
            model['*controlled'] = self.var_engine.get_uncontrolled()
        return status, model, None
