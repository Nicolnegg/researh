...
class SVCompRuleSet(GenericRuleSet):
    ...
    def write_abduction_runner(self, stream, config, rconfig, memory, binary, literals, directives, asmaddr, timeout, autocontrol=False, ct_mode=False, stack=[]):
        ...
        stream.write('local_pyabduce=""\n')
        stream.write('local_tools_root=""\n')
        stream.write('local_pythonpath="${PYTHONPATH:-}"\n')
        stream.write('for rel in "../../../../tools" "../../../tools"; do\n')
        stream.write('  cand="${script_dir}/${rel}/pyabduce/pyabduce"\n')
        stream.write('  if [[ -x "$cand" ]]; then\n')
        stream.write('    local_pyabduce="$cand"\n')
        stream.write('    local_tools_root="${script_dir}/${rel}"\n')
        stream.write('    break\n')
        stream.write('  fi\n')
        stream.write('done\n')
        stream.write('if [[ -d "${local_tools_root}/pulseutils" ]]; then\n')
        stream.write('  local_pythonpath="${local_tools_root}/pulseutils${local_pythonpath:+:${local_pythonpath}}"\n')
        stream.write('elif [[ -d "${local_tools_root}/fistic/local/pulseutils" ]]; then\n')
        stream.write('  local_pythonpath="${local_tools_root}/fistic/local/pulseutils${local_pythonpath:+:${local_pythonpath}}"\n')
        stream.write('fi\n')
        stream.write('if [[ -z "${PYABDUCE:-}" && -n "$local_pyabduce" ]]; then\n')
        stream.write('  if PYTHONPATH="${local_pythonpath}" "$local_pyabduce" --help >/dev/null 2>&1; then\n')
        stream.write('    PYABDUCE="$local_pyabduce"\n')
        stream.write('    export PYTHONPATH="${local_pythonpath}"\n')
        stream.write('  fi\n')
        stream.write('fi\n')
        ...
        if autocontrol:
            stream.write('if [[ "${ABDUCE_HIDE_BINSEC_TIMES:-1}" = "1" ]]; then\n')
            stream.write('  set -o pipefail\n')
            stream.write('  "${{PYABDUCE:-pyabduce}}" --binsec-config {} --binsec-memory {} --binsec-binary {} --binsec-addr {} --literals {} --binsec-directives {} --binsec-timeout {} --binsec-robust --robust-config {} {}$@ 2>&1 | sed \'/\\* binsec times:/d\'\n'.format(config, memory, binary, asmaddr, literals, directives, timeout, rconfig, ctarg))
            stream.write('  exit "${PIPESTATUS[0]}"\n')
            stream.write('fi\n')
            ...
            stream.write('if [[ "${ABDUCE_HIDE_BINSEC_TIMES:-1}" = "1" ]]; then\n')
            stream.write('  set -o pipefail\n')
            stream.write('  "${{PYABDUCE:-pyabduce}}" --binsec-config {} --binsec-memory {} --binsec-binary {} --binsec-addr {} --literals {} --binsec-directives {} --binsec-timeout {} {}$@ 2>&1 | sed \'/\\* binsec times:/d\'\n'.format(config, memory, binary, asmaddr, literals, directives, timeout, ctarg))
            stream.write('  exit "${PIPESTATUS[0]}"\n')
            stream.write('fi\n')
